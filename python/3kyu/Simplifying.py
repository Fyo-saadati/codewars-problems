...
def parenthesesRemover(formula):
    ...
    if (number1 := parenthese[: parenthese.index("(")]) in {
        "+": "1",
        "-": "-1",
        "": "1",
        "1": "+1",
    ...
    allVariable = [
        x
        for x in re.findall(
            "([-+]*[0-9]*[a-zA-Z]*)", parenthese[parenthese.index("(") :]
        )
        if x != ""
    ]
    ...
    return [
        "".join([x if x[0] == "-" else f"+{x}" for x in allVariable]).replace(
            "++", "+"
        ),
        parenthese,
    ]
...
def computing(finalString):
    for i, par in enumerate(
        allVar := [
            x for x in re.findall("([+-]*[0-9]*[a-zA-Z]*)", finalString) if x != ""
        ]
    ):
    ...
    result = ""
    sortedVar = [
        [x for x in allVar if y in x]
        for y in set(re.findall("([a-zA-Z])", finalString))
    ]
        ...
        addition = sum(
            list(
                map(
                    ...
                    [x for x in re.findall("([+-]*[0-9]*)", "".join(lst)) if x != ""],
                )
            )
        )
        if addition > 0:
            result += (
                f"+{addition}{''.join(x for x in lst[0] if x not in'+-0123456789')}"
            )
        else:
            result += (
                f"{addition}{''.join(x for x in lst[0] if x not in'+-0123456789')}"
...
def parenthesesFinder(formula):
    ...
    return [
        (newString := formula[hint.start() :])[: newString.index(")") + 1] for hint in n
    ]
...
def examplesSimplify(examples):
    pattern = {
        example[-1]: example[:-2]
        for example in ["".join(string.split()) for string in examples]
    }
